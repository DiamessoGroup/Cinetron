...
app.controller('myCtrl', function ($scope, $http, $sce) {
  ...
  let randomPage = getRandomIntInclusive(1, 7);
  ...
  $http({
    ...
    var i = 0;
    $scope.trailerKey = $scope.discoverMovie.results[i].id;
    ...
    $scope.nextMovie = function () {
      ...
      $http({
        method: "GET",
        url: `http://api.themoviedb.org/3/movie/${$scope.trailerKey}?api_key=a862696eab5e9103df57baedbe9d56a9&append_to_response=videos`
      }).then(function mySuccess(response) {
        ...
        for (i = 0; i < $scope.trailerDetail.videos.results.length; i += 1) {
          if ($scope.trailerDetail.videos.results[i].type == 'Trailer' && $scope.trailerDetail.videos.results[i].site == 'YouTube') {
            ...
            $scope.youtubeLinkFull = $sce.trustAsResourceUrl(`https://www.youtube.com/embed/${$scope.trailerDetail.videos.results[i].key}?autoplay=1&amp;rel=0&amp;controls=0&amp;hd=1&amp;iv_load_policy=3&amp;showinfo=0`);
            break;
          }
        }
